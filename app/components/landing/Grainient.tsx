...
// React Bits Grainient. TypeScript + "use client" added (required
// here); shader and the two-effect renderer/uniform split are
// unchanged. one fix: the built-in IntersectionObserver does not track
// GSAP ScrollSmoother's transform-based scroll (so it never animated),
// replaced with a ScrollSmoother-aware ScrollTrigger. still pauses
...
export function Grainient({
  ...
  useEffect(() => {
    ...
    // ScrollSmoother moves content via transform, which IntersectionObserver
    // does not reliably track, so the original IO never reported visible
    // and the loop stayed stopped. ScrollTrigger is ScrollSmoother-aware.
